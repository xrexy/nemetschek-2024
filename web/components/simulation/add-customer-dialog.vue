  ...
  <Dialog v-if="data" v-model:open="isOpen">
    <DialogContent class="sm:max-w-[375px] grid-rows-[auto_minmax(0,1fr)_auto] p-0">
      <DialogHeader class="p-6 pb-0">
        ...
        <DialogDescription>
          Add a new customer to <span class="text-emerald-200">{{ data.slug }}</span>
        </DialogDescription>
      </DialogHeader>
      <div class="border-t border-border flex flex-col gap-4 py-4 overflow-y-auto px-6">
        <FormKit type="form" :actions="false" @submit="onSubmit">
          <div class="w-full sm:flex-row gap-x-2">
            <FormKit type="text" label="Name" name="name" step="1" validation="required" placeholder="Customer name" />
...
const { data, isOpen, close } = useAddCustomerDialog();
...
async function onSubmit(e: any) {
  ...
  data.value.add({
    ...
    id: data.value.customers.length,
  })
